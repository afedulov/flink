...
class StackTraceSampleService {
    ...
    private Optional<StackTraceElement[]> getStackTrace(
            final StackTraceSampleableTask task, final int maxStackTraceDepth) {
        if (!task.isRunning()) {
    ...
    private StackTraceElement[] truncateStackTrace(
            final StackTraceElement[] stackTrace, final int maxStackTraceDepth) {
        if (maxStackTraceDepth > 0) {
            return Arrays.copyOfRange(
                    stackTrace, 0, Math.min(maxStackTraceDepth, stackTrace.length));
        }
        return stackTrace;
    }
}
