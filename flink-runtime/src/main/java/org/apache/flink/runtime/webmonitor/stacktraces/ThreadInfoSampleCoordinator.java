...
public class ThreadInfoSampleCoordinator {
     ...
     * @param maxStackTraceDepth Maximum depth of the stack trace. 0 indicates no maximum and keeps
    ...
    public CompletableFuture<ThreadInfoSample> triggerThreadInfoSample(
        ...
        synchronized (lock) {
            ...
            final PendingStackTraceSample pending =
                    new PendingStackTraceSample(sampleId, triggerIds);
    ...
    public void collectStackTraces(
            int sampleId, ExecutionAttemptID executionId, List<ThreadInfo> stackTraces) {
        ...
        synchronized (lock) {
            ...
            if (pending != null) {
                ...
                if (LOG.isDebugEnabled()) {
                    LOG.debug(
                            "Received late stack trace sample {} of task {}",
                            sampleId,
                            executionId);
                }
            } else {
    ...
    /**
     * A pending stack trace sample, which collects stack traces and owns a {@link ThreadInfoSample}
     * promise.
     *
    ...
    private static class PendingStackTraceSample {
        ...
        PendingStackTraceSample(int sampleId, List<ExecutionAttemptID> tasksToCollect) {
        ...
        void collectStackTraces(ExecutionAttemptID executionId, List<ThreadInfo> stackTraces) {
            ...
            if (pendingTasks.remove(executionId)) {
                threadInfoSubSamplesByTask.put(
                        executionId, Collections.unmodifiableList(stackTraces));
            } else if (isComplete()) {
        ...
        void completePromiseAndDiscard() {
            if (isComplete()) {
                ...
                ThreadInfoSample stackTraceSample =
