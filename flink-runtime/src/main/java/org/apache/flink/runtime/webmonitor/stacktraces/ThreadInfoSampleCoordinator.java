...
public class ThreadInfoSampleCoordinator {
	...
	public CompletableFuture<ThreadInfoSample> triggerThreadInfoSample(
		...
		synchronized (lock) {
			...
			// Trigger all samples
			for (Tuple2<AccessExecutionVertex, CompletableFuture<TaskExecutorGateway>> executionWithGateway : executionsWithGateways) {
	...
	private static class PendingStackTraceSample {
		...
		PendingStackTraceSample(
				int sampleId,
				List<ExecutionAttemptID> tasksToCollect) {
